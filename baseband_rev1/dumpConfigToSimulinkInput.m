...
simInputs(configInd) = simInputs(configInd).setVariable('RxEQ2Partition', RxEQ2Partition);
...
%Sub-Blocking
simInputs(configInd) = simInputs(configInd).setVariable('TxResampleModulatorSubBlocking', TxResampleModulatorSubBlocking);
simInputs(configInd) = simInputs(configInd).setVariable('TxRRCFilterSubBlocking', TxRRCFilterSubBlocking);
simInputs(configInd) = simInputs(configInd).setVariable('RxRRCSubBlocking', RxRRCSubBlocking);
simInputs(configInd) = simInputs(configInd).setVariable('RxAGCPwrAvgSubBlocking', RxAGCPwrAvgSubBlocking);
simInputs(configInd) = simInputs(configInd).setVariable('RxAGCCorrectionLoopSubBlocking', RxAGCCorrectionLoopSubBlocking);
simInputs(configInd) = simInputs(configInd).setVariable('RxTimingRecoveryGolayCorrelatorSubBlocking', RxTimingRecoveryGolayCorrelatorSubBlocking);
simInputs(configInd) = simInputs(configInd).setVariable('RxTimingRecoveryGolayPeakDetectSubBlocking', RxTimingRecoveryGolayPeakDetectSubBlocking);
simInputs(configInd) = simInputs(configInd).setVariable('RxTimingRecoveryControlSubBlocking', RxTimingRecoveryControlSubBlocking);
simInputs(configInd) = simInputs(configInd).setVariable('RxTimingRecoveryCalcDelayErrorSubBlocking', RxTimingRecoveryCalcDelayErrorSubBlocking);
simInputs(configInd) = simInputs(configInd).setVariable('RxTimingRecoveryFreqEstSubBlocking', RxTimingRecoveryFreqEstSubBlocking);
simInputs(configInd) = simInputs(configInd).setVariable('RxTimingRecoveryDelayAccumSubBlocking', RxTimingRecoveryDelayAccumSubBlocking);
simInputs(configInd) = simInputs(configInd).setVariable('RxTimingRecoveryVariableDelaySubBlocking', RxTimingRecoveryVariableDelaySubBlocking);
simInputs(configInd) = simInputs(configInd).setVariable('RxTimingRecoverySymbolClockSubBlocking', RxTimingRecoverySymbolClockSubBlocking);
simInputs(configInd) = simInputs(configInd).setVariable('RxTimingRecoveryEarlyLateSubBlocking', RxTimingRecoveryEarlyLateSubBlocking);
simInputs(configInd) = simInputs(configInd).setVariable('RxSymbGolayCorrelatorSubBlocking', RxSymbGolayCorrelatorSubBlocking);
simInputs(configInd) = simInputs(configInd).setVariable('RxSymbGolayPeakDetectSubBlocking', RxSymbGolayPeakDetectSubBlocking);
simInputs(configInd) = simInputs(configInd).setVariable('RxCoarseCFOSubBlocking', RxCoarseCFOSubBlocking);
simInputs(configInd) = simInputs(configInd).setVariable('RxEQSubBlocking', RxEQSubBlocking);
simInputs(configInd) = simInputs(configInd).setVariable('RxEQ2SubBlocking', RxEQ2SubBlocking);
simInputs(configInd) = simInputs(configInd).setVariable('RxFineCFOSubBlocking', RxFineCFOSubBlocking);
simInputs(configInd) = simInputs(configInd).setVariable('RxFineCFOCorrectComputeSubBlocking', RxFineCFOCorrectComputeSubBlocking);
simInputs(configInd) = simInputs(configInd).setVariable('RxDemodSubBlocking', RxDemodSubBlocking);
simInputs(configInd) = simInputs(configInd).setVariable('RxPackerSubBlocking', RxPackerSubBlocking);
simInputs(configInd) = simInputs(configInd).setVariable('RxPacketControllerSubBlocking', RxPacketControllerSubBlocking);
simInputs(configInd) = simInputs(configInd).setVariable('RxFreezeControllerSubBlocking', RxFreezeControllerSubBlocking);
%Other Params
...
simInputs(configInd) = simInputs(configInd).setVariable('headerLenSymbols', headerLenSymbols);
...
simInputs(configInd) = simInputs(configInd).setVariable('modFieldLenSymbols', modFieldLenSymbols);
...
simInputs(configInd) = simInputs(configInd).setVariable('unrollFactor', unrollFactor);
...
simInputs(configInd) = simInputs(configInd).setVariable('pktLenPlusCEF', pktLenPlusCEF);
...
simInputs(configInd) = simInputs(configInd).setVariable('eqBatchSize', eqBatchSize);
simInputs(configInd) = simInputs(configInd).setVariable('eqPipeline', eqPipeline);
simInputs(configInd) = simInputs(configInd).setVariable('eqTrainingEarlyShutoff', eqTrainingEarlyShutoff);
...
simInputs(configInd) = simInputs(configInd).setVariable('qam256_points', qam256_points);
simInputs(configInd) = simInputs(configInd).setVariable('qam256_power_normalized_distance', qam256_power_normalized_distance);
simInputs(configInd) = simInputs(configInd).setVariable('qam256_hdl_distance', qam256_hdl_distance);
simInputs(configInd) = simInputs(configInd).setVariable('qam256_demod_scale_factor', qam256_demod_scale_factor);
...
simInputs(configInd) = simInputs(configInd).setVariable('cfoEarlyWarningStartTraining', cfoEarlyWarningStartTraining);
simInputs(configInd) = simInputs(configInd).setVariable('cfoEarlyWarningReset', cfoEarlyWarningReset);
